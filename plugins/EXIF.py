...
class Exif_Analyser:
    ...
    def extract_metadata(self, path_to_file: str) -> dict:
        ...
        metadata = self._enrich_with_GPS_Information(metadata=metadata)
    ...
    def _extract_metadata(self, path_to_file):
        try:
            ...
            try:
                ...
                metadata = self._decode_metadata(metadata=metadata)
                return metadata
    ...
    def _decode_metadata(self, metadata: dict) -> dict:
        decoded = dict()
    ...
    def _enrich_with_GPS_Information(self, metadata: dict):
        if 'GPSInfo' in metadata:
            gps_info = self._decode_GPSInfo(gps_info_encoded=metadata['GPSInfo'][0])
            for key, value in gps_info.items():
                metadata['GPSInfo:{0}'.format(key)] = (value, 'extracted from EXIF GPSInfo metadata')
            if self._gps_info_contains_GPS_position(decoded_gps_info=gps_info):
                lat, lat_ref, lon, lon_ref = self._decode_GPS_position(decoded_gps_info=gps_info)
            ...
            if self._gps_info_contains_altitude(decoded_gps_info=gps_info):
                altitude, alt_ref = self._decode_GPS_altitude(decoded_gps_info=gps_info)
    ...
    def _decode_GPSInfo(self, gps_info_encoded: dict):
        gps_info = dict()
    ...
    def _gps_info_contains_GPS_position(self, decoded_gps_info: dict):
    ...
    def _decode_GPS_position(self, decoded_gps_info: dict):
        try:
            lat = self._convert_degree_to_decimal(decoded_gps_info['GPSLatitude'])
            lat_ref = decoded_gps_info['GPSLatitudeRef']
            lon = self._convert_degree_to_decimal(decoded_gps_info['GPSLongitude'])
            lon_ref = decoded_gps_info['GPSLongitudeRef']
            return lat, lat_ref, lon, lon_ref
        except:
            return None

    def _gps_info_contains_altitude(self, decoded_gps_info: dict):
        return 'GPSAltitude' in decoded_gps_info and 'GPSAltitudeRef' in decoded_gps_info 

    def _decode_GPS_altitude(self, decoded_gps_info: dict):
    ...
    def _convert_degree_to_decimal(self, value):
